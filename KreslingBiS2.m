function [Node, Panel,V2,Vd,Hd, Rd] = KreslingBiS2(phi1,phi2,r,n,m,Output)
 %S2     
...
Node1=Node(1:2*n,:);
A0=[0 0];
 %calculate C1 :the intersection point of the projection of A1B2 and A2B3
   X1=Node1(1,1:2);
   Y1=Node1(n+2,1:2);
   X2=Node1(2,1:2);
   Y2=Node1(n+3,1:2);
...
if X1(1)~=Y1(1)&X2(1)~=Y2(1)
  ...
  Vd=n*h*m*(norm(A0-C))^2*sin(pi/n)*cos(pi/n);   
...
V2=((Vadd1+Vadd2+Vadd3)*2+Vadd4)*n*m;
else
    
Vmath=n*R^2*h*sin(pi/n)*cos(pi/n)*2/3;
%[Vadd]=calvolume(Node1(1,:),Node1(n+1,:),Node1(n+2,:),Node1(3*n+1,:));
Vaddmath=1/6*R^2*h*abs((sin(theta)-sin(theta+2*pi/n)));
V2=m*(Vmath+n*Vaddmath);
end
