function [Node, Panel,V2,Vd, Hd, Rd] = MiuraBiS2(phi1,phi2,r,n,m,d1overr,Output)
%  n=4
%  m=3
%  phi1=75/180*pi
%  phi2=32/180*pi
...
V2=(V1+V2+V3+V4+V5+V6)*m*n;

%judge line A2B2 and line A4B4 has an intersection point
x=[Node1(2,1) Node1(8,1) Node1(10,1) Node1(12,1)];
y=[Node1(2,2) Node1(8,2) Node1(10,2) Node1(12,2)];
...
if(dt1<=0 & dt2<=0)
    ...
    % intersection point of the projection of A2B2 and A4B4
    X1=Node1(2,1:2);
    Y1=Node1(5,1:2);
    X2=Node1(10,1:2);
    Y2=Node1(12,1:2);
    ...
    Vd=n*m*h*norm(Node1(6*n+3,1:2)-C(1,1:2))^2*sin(pi/n)*cos(pi/n);
else
    intrsct=0;
    %calculate the intersection point of the projection of A3A4 and B2B3
    X1=Node1(3,1:2);
    Y1=Node1(10,1:2);
    X2=Node1(5,1:2);
    Y2=Node1(6,1:2);
    ...
    p1=(norm(Node1(6*n+3,1:2)-Node1(2,1:2))+norm(Node1(6*n+3,1:2)-Node1(5,1:2))+norm(Node1(2,1:2)-Node1(5,1:2)))/2;
    S1=sqrt(p1*(p1-norm(Node1(6*n+3,1:2)-Node1(2,1:2)))*(p1-norm(Node1(6*n+3,1:2)-Node1(5,1:2)))*(p1-norm(Node1(2,1:2)-Node1(5,1:2))));
    p2=(norm(Node1(6*n+3,1:2)-Node1(5,1:2))+norm(Node1(6*n+3,1:2)-C(1,1:2))+norm(Node1(5,1:2)-C(1,1:2)))/2;
    S2=sqrt(p2*(p2-norm(Node1(6*n+3,1:2)-Node1(5,1:2)))*(p2-norm(Node1(6*n+3,1:2)-C(1,1:2)))*(p2-norm(Node1(5,1:2)-C(1,1:2))));
    Vd=m*n*h*(S1+2*S2);
...
filename = sprintf('Miura_S2.FOLD');
